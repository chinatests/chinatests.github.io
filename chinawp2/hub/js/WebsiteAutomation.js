...
function processData(msg)
{
    ...
    if(isDomainSaved === true)
    {
        ...
        window.location.href=protocol+"http://"+ZH_URL+"/marketinghub/WebAutoClose.do";
    } 
}
...
function oReq(value,isHashChng) {
    ...
    if(testBoolean === true)
    {
        ...
        if(tmpdomainId.indexOf(testId)>-1 || aliasDomainBoolean === true)
        {
            ...
            for(var i=0;i<domainIds.length;i++)
            {
                ...
                ajaxUrl = protocol+"http://"+ZH_URL+"/marketinghub/AuthenticateDomain.do"+"?callback=processData"+actionData; //No I18N
                makeAjaxReq(ajaxUrl);
            }
        }
    }
    else if(getZCookie("zc_consent") == 1)
    {
        ...
        for(var i=0;i<webAutoIds.length;i++)
        {
            ...
            ajaxUrl = protocol+"http://"+ZC_RedirUrl+"/wa/ActionLogger"+"?callback=processData"+paramData; //No I18N
            makeAjaxReq(ajaxUrl);
        }
    }
}
   ...
   if((( action != null && action != "") || ( value != null && value != "")) && cookieEnabled)
    {
        ...
        if(testBoolean === true)
        {
            if(domainIds.indexOf(testId)>-1)
            {
                ...
                ajaxUrl = protocol+"http://"+ZC_RedirUrl+"/marketinghub/AuthenticateDomain.do"+"?callback=processData"+actionData; //No I18N
                makeAjaxReq(ajaxUrl);
            }
        }
        else if(getZCookie("zc_consent")==1)
...
function waListSubscribe(contactInfo, listKey)
{
   if(cookieEnabled)
   {
    ...
    if(contactInfo != null && contactInfo !== "" && listKey != null && listKey !== "")
    {
        ...
        ajaxUrl = protocol+"http://"+ZC_RedirUrl+"/api/setleads"+"?callback=processData&"+stringParams; //No I18N
        makeAjaxReq(ajaxUrl);
    }
}
}
...
function zcListSubscribe(contactInfo, listKey)
{
   if(cookieEnabled)
    ...
    if(contactInfo != null && contactInfo !== "" && listKey != null && listKey !== "")
    {
        ...
        contactParams.zuid = zuid;
        stringParams = serializeMHJson(contactParams);
        ajaxUrl = protocol+"http://"+ZH_URL+"/api/v1/Mhlistsubscribefromwebsite"+"?callback=processData&"+stringParams; //No I18N
        makeAjaxReq(ajaxUrl);
    }
}
}
...
function zcListSubscribeCallback(data)
{
 if(cookieEnabled)
    ...
    if(anonymousCookie != null && anonymousCookie !== "")
    {
        ...
        ajaxUrl = protocol+"http://"+ZC_RedirUrl+"/wa/addmappingforanonymousandcontacts"+"?callback=processData&"+stringParams; //No I18N
        makeAjaxReq(ajaxUrl);
    }
}
}
...
function zcFieldUpdate(json)
{
  if(cookieEnabled)
    ...
    ajaxUrl = protocol+"http://"+ZH_URL+"/wa/MhSavecookie"+"?callback=processData&"+stringParams; //No I18N
    makeAjaxReq(ajaxUrl);
}
}
...
function setTabChange()
{
    ...
    //          document.cookie = "zc_ssid" + '=;expires=Thu, 01 Jan 1970 00:00:01 GMT;';
                ...
                actionData = actionData+"&parentUrl="+encodeURIComponent(parentUrl);//No I18N
                var paramData = actionData;
                ajaxUrl = protocol+"http://"+ZC_RedirUrl+"/wa/ActionRevisitLogger"+"?callback=processData"+paramData; //No I18N
                makeAjaxReq(ajaxUrl);
                fnSetTimer();
            }
            else
...
var tabChange = (function()
{
    var stateKey, eventKey, keys = {
        ...
        msHidden: "msvisibilitychange"http://No I18N
    };
...
if(cookieEnabled)
    ...
    param.zuid = zuid;
    stringParams = serializeMHJson(param);
    ajaxUrl = protocol+"http://"+ZH_URL+"/wa/MhSetItem"+"?callback=processData&"+stringParams; //No I18N
    makeAjaxReq(ajaxUrl); 
}
}
...
var watrackSignupEvent = function (formAction) {
    ...
    if (campaignsUrl != undefined && campaignsUrl.indexOf("http") < 0) {
        ...
        campaignsUrl = protocol + "http://" + campaignsUrl;
    }
